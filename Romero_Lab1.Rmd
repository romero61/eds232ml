...
**It is not very useful as we can't distinguish the different bushel types.**
...
Now let's introduce another useful Tidymodels package: recipes - which will help you preprocess data before training your model.

**A recipe is an object that defines what steps should be applied to a data set in order to get it ready for modelling.**
...
The call to recipe() with a formula tells the recipe the roles of the variables using new_pumpkins data as the reference.

For instance the **price column has been assigned an outcome role while the rest of the columns have been assigned a predictor role**.
...
How can we confirm that the recipe is doing what we intend?

Once your recipe is defined, you can estimate the parameters required to preprocess the data, and then extract the processed data.

You don't typically need to do this when you use Tidymodels (we'll see the normal convention in just a minute with workflows) but its a good sanity check for confirming that recipes are doing what you expect.
...
***Ten cities are represented in the data set.***
...
***The correlation between the package and the price is most highly correlated with price. Larger bundle sizes will correlate with higher prices and smaller bundles will correlate with lower prices.***
...
***There is a lack of variation in the distribution of the data and the data does not look normally distributed.***
