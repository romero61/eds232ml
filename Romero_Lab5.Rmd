...
predict <- as.data.frame(final_tree_fit$.predictions) |> select(-.config)
...
# Random Forest
    -   rand_forest()
    -   m_try() is the new hyperparameter of interest for this type of model. Make sure to include it in your tuning process Go through the modeling process for each model: Preprocessing. You can use the same recipe for all the models you create. Resampling. Make sure to use appropriate resampling to select the best version created by each algorithm. Tuning. Find the best values for each hyperparameter (within a reasonable range). Compare the performance of the four final models you have created.\
...
rf_auc0 <- 
  ...
  roc_curve(data_owned_by, .pred_0) %>% 
...
rf_auc1 <- 
  rf_res %>% 
  collect_predictions(parameters = best_rf) %>% 
  roc_curve(data_owned_by, .pred_1) %>% 
  mutate(model = "Random Forest")
...
ggplot(rf_auc, aes(x = 1 - specificity, y = sensitivity, col = model)) + 
  geom_path(linewidth = 1.5, alpha = 0.8) +
  geom_abline(lty = 3) + 
  coord_equal() + 
  scale_color_viridis_d(option = "plasma", end = .6) +
  ggplot(rf_auc2, aes(x = 1 - specificity, y = sensitivity, col = model)) + 
  geom_path(linewidth = 1.5, alpha = 0.8) +
  geom_abline(lty = 3) + 
  coord_equal() + 
  scale_color_viridis_d(option = "plasma", end = .6) 
