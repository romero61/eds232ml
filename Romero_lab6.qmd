...
xgb_spec3 <- boost_tree(
  mtry = 19,
  trees = 816,  #  the number of trees
  tree_depth = 7,  #  the maximum depth of each tree
  min_n = 1,  # the minimum number of observations in each terminal node
  loss_reduction = 1.130081,  #  the minimum loss reduction required to make a further partition on a leaf node
...
xgb_spec_final <- boost_tree(
  ...
  stop_iter = 8,  # Early stopping parameter
  learn_rate = 0.01044138  # Set the optimal learning rate
) %>% 
  set_engine("xgboost") %>% 
  set_mode("classification")
...
eel_predict <- final_fit |> 
  predict(data = data_test)
...
final_pred_test <- predict(final_fit, data_test)
```
...
# Create a recipe for the data
eel_eval_recipe <- recipe(Angaus ~ ., data = data_train) %>%
  step_normalize(all_numeric(), -all_outcomes()) %>%
  step_dummy(all_nominal(), -all_outcomes())
